...
<div class="wrapper row5">
  <div id="container" class="clear"> 
    ...
    <br/>
    <form id="frmblock" name="frmblock" action="pproject_search.php" method="post">
	...
	if(mysql_query($Insertquery))
	{
		//Finished  Pending   Correction
			$sqlDept="SELECT * FROM `tbl_pendproject` WHERE `pendprj_cat`='$usr_cat' AND `pendprj_name` LIKE '%$b%'";
			//print $sqlDept;    
        ...
        <?php
			...
			else
			{
				...
				{	
                //`pendprj_id``pendprj_uid``pendprj_cat``pendprj_name``pendprj_lang``pendprj_ide``pendprj_time`
				//`corrprj_file` `crtprj_file`  `fnprj_file`  `pendprj_file`
				$res=mysql_fetch_array($rstDept);
				$pr_id=$res["pendprj_id"];
				$pr_category=$res["pendprj_cat"];
				$pr_name=$res["pendprj_name"];
				$pr_language=$res["pendprj_lang"];
				$pr_duration=$res["pendprj_time"];
				$pr_description=$res["pendprj_ide"];
				$pr_file=$res["pendprj_file"];
