...
<div class="wrapper row5">
  <div id="container" class="clear"> 
    ...
    <br/>
    <form id="frmblock" name="frmblock" action="crproject_search.php" method="post">
      <table style="color:#333; font-size:14px;">
        <tr>
          <td width="117">Project name</td>
          <td width="376"><input type="hidden" name="txt_cat" id="txt_cat" value="<?php print $cat;  ?>" /><input type="text" name="dd_branch" id="dd_branch" class="ddlist" onchange="submitbranch();" /></td>
        </tr>
      </table>
      </form>
      </fieldset>
	...
	if(mysql_query($Insertquery))
	{
		//Finished  Pending   Correction
			$sqlDept="SELECT * FROM `tbl_corrproject` WHERE `corrprj_cat`='$usr_cat' AND `corrprj_name` LIKE '%$b%'";
			//print $sqlDept;    
        ...
        <?php
			...
			else
			{
				...
				{	
                //`corrprj_id``corrprj_uid``corrprj_cat``corrprj_name``corrprj_lang``corrprj_ide``corrprj_time`
				//`corrprj_file` `crtprj_file`  `fnprj_file`  `pendprj_file`
				$res=mysql_fetch_array($rstDept);
				$pr_id=$res["corrprj_id"];
				$pr_category=$res["corrprj_cat"];
				$pr_name=$res["corrprj_name"];
				$pr_language=$res["corrprj_lang"];
				$pr_duration=$res["corrprj_time"];
				$pr_description=$res["corrprj_ide"];
				$pr_file=$res["corrprj_file"];
